...
class GeoAwarePooling(BaseModule):
    ...
    def scatter_norm(self, points, idx):
        ...
        center_segment = scatter(points, idx, dim=0, reduce='mean')         # this center presents the superpoint center `c_i`
    ...
    def forward(self, x, sp_idx, all_xyz, with_xyz=False):
        """
        Args:
            - x: Mink backbone features of all points 
            - sp_idx: according to mask, point from the same superpoint has the same index
            - all_xyz: coordinates of all points after `ME.utils.batch_sparse_collate` and `ME.TensorField`
        """
        all_xyz_ = torch.cat(all_xyz)
        all_xyz, _ = self.scatter_norm(all_xyz_, sp_idx)  # sp_idx: superpoints index
...
if __name__ == '__main__':
    pool = GeoAwarePooling(16)
    x = torch.randn(10, 32)
    sp_idx = torch.randint(0, 5, (10,))
    all_xyz = [torch.randn(4, 3), torch.randn(3, 3), torch.randn(3, 3)]
    out, w = pool(x, sp_idx, all_xyz, with_xyz=True)
