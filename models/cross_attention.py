...
if __name__ == "__main__":
    ...
    def test_cross_attention():
        ...
        # 模拟输入
        batch_size = 2
        n_queries = 1024         # DINOv3 patch tokens (e.g. 16x16)
        n_points = 60000         # 3D point cloud tokens
        d_model = 768
        num_heads = 8
        dropout = 0.1

        # 构造 DINOv3 tokens 和 3D tokens
        dino_tokens = [torch.randn((n_queries, d_model), device=device) for _ in range(batch_size)]  # 256 x 768
        pc_tokens = [torch.randn((n_points, d_model), device=device) for _ in range(batch_size)]     # 1024 x 768

        # 初始化 CrossAttentionLayer
        cross_attn = CrossAttentionLayer(d_model=d_model, num_heads=num_heads, dropout=dropout, fix=True).to(device)

        # 前向传播
        fused_tokens = cross_attn(sources=pc_tokens, queries=dino_tokens)

        # 输出结果
        for i, out in enumerate(fused_tokens):
            print(f"Batch {i}: output shape = {out.shape}")
